...
/**
  ...
  static async signup(req, res) {
    try {
      ...
      const confirmEmailPage = `${process.env.FRONTEND_URL}confirmation/${token}`;
  ...
  static async updatedUser(req, res) {
    ...
    const { email } = payload;
    const updateUser = await UserServices.activeUser(email, activate);

    if (updateUser.status === 200) {
      return response.successMessage(res, updateUser.message, updateUser.status, 'Hurray you can now login');
    }
